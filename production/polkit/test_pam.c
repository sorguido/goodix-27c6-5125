...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *p, int f, int n, const char **a)
{
  ...
  const char *marker = getenv("GOODIX_POLKIT_TEST_MARKER");
  if (!marker || strncmp(marker, "/tmp/goodix-polkit-test.", 24)) return PAM_AUTH_ERR;
  ...
  const char *mode = getenv("GOODIX_POLKIT_TEST_MODE");
...
PAM_EXTERN int pam_sm_acct_mgmt(pam_handle_t *p, int f, int n, const char **a)
{
  ...
  return getenv("GOODIX_POLKIT_TEST_ACCOUNT_FAIL") ? PAM_PERM_DENIED : PAM_SUCCESS;
}
#else
...
{
  if (argc != 3) return 2;
  if (getenv("GOODIX_POLKIT_TEST_COOKIE")) {
    char dummy[128];
    if (!fgets(dummy, sizeof dummy, stdin)) return 2;
  }
  struct pam_conv conv = {conversation, NULL};
  pam_handle_t *p = NULL;
  int r = pam_start_confdir(argv[2], getpwuid(getuid())->pw_name, &conv, argv[1], &p);
  if (r == PAM_SUCCESS) r = pam_authenticate(p, 0);
