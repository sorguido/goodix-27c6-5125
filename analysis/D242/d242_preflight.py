"""D242 root preflight; no libusb call and no sensor traffic."""
...
OUTPUT = REPOSITORY / "analysis/D242/D242_preflight_report.json"
D242_MARKER_NAME = "d242-operator-invocation.marker"
...
def marker_namespace_status(store: Path) -> dict[str, object]:
    current = store / D242_MARKER_NAME
    return {
        "namespace": str(current),
        "d242_marker_absent": not os.path.lexists(current),
...
def offline_import_probe() -> int:
    ...
    paths.validate()
    report = {
        "schema": "d242-preflight-import-probe-v1",
        "status": "pass",
    ...
    if paths.single_use_marker.name != D242_MARKER_NAME:
        report["status"] = "fail"
...
def offline_sandbox_preflight(root: Path) -> dict[str, object]:
    ...
    return {
        "schema": "d242-offline-preflight-fixture-v1",
        "status": "PASS"
        if first["d242_marker_absent"]
        and not second["d242_marker_absent"]
        and directory["pass"]
        else "FAIL",
        "historical_markers_case": first,
        "consumed_d242_marker_case": second,
...
def live_preflight() -> int:
    # Reuse the already reviewed protected-input/sysfs/service checks while
    # substituting only the D242 output and marker namespace.
    base.OUTPUT = OUTPUT
    base.HISTORICAL_MARKER_NAMES = HISTORICAL_MARKER_NAMES
    base.D241_MARKER_NAME = D242_MARKER_NAME
    status = base.live_preflight()
    report = json.loads(OUTPUT.read_text(encoding="utf-8"))
    report["schema"] = "d242-preflight-report-v1"
    marker = report.get("marker_namespace_status", {})
    if "d241_marker_absent" in marker:
        marker["d242_marker_absent"] = marker.pop("d241_marker_absent")
...
def main(argv: tuple[str, ...] | None = None) -> int:
    ...
    if len(arguments) == 2 and arguments[0] == "--offline-sandbox-preflight":
        ...
        return 0 if report["status"] == "PASS" else 1
    if arguments:
        print("d242_preflight: unexpected arguments", file=sys.stderr)
        return 64
