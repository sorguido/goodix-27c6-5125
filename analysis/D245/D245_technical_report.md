...
Il gate accetta solo ACK `B0/A8/01` seguito dalla response tipizzata esatta
`GF_ST411SEC_APP_12509\0`. ACK07 termina senza autorizzare la response; altri
status, mismatch, timeout, ordine, wrapper, checksum e stale buffered frame
terminano con E4 non inviato. Il boundary `A8_COMPLETE → E4_OUT` è verificato
dalla phase trace. A8 resta un discriminatore/precondizione read-only e non è
interpretato come initializer, reset o wake.
...
Ogni terminale della matrice (A8 OUT timeout, A8 IN timeout, ACK07, due firmware
mismatch, stale/malformed/order, E4 IN timeout, TLS timeout e TLS success) passa
cleanup exactly-once, zeroizzazione, checkpoint pre-restore, restore esatto di
`fprintd`/signal mask e report finale. Il caso `fprintd` inizialmente inactive
verifica che non venga avviato artificialmente.

## Root rename e launcher

`d245-live-tls-once.sh` risolve la root dal proprio path e da Git, poi pinna
contenuti e hash; non dipende dal basename. Il dry-run usa identici cwd,
`PYTHONPATH`, patch e composition chain della futura branch live, ma soltanto in
una copia temporanea con fake USB. I launcher storici D239–D244 hanno ricevuto
la sola correzione host-side equivalente di root resolution, necessaria affinché
la suite storica raggiunga i propri gate dopo il rename; wire, marker, bundle e
sorgenti sealed storici non sono stati modificati.
...
matrice prova 13 comandi logici nel path completo (A8 più i 12 pre-D1), E4
ACK01/07, B0 fisici `64|64|64`, due pacing e stop prima di D4.
