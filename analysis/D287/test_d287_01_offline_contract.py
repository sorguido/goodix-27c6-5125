...
class D287OfflineContract(unittest.TestCase):
    ...
    def test_04_target_versions_and_installed_hashes_are_pinned(self):
        ...
        for marker in (
                "kscreenlocker-6.7.4-1.fc44.x86_64",
                "plasma-workspace-6.7.4-2.fc44.x86_64",
    ...
    def test_07_host_pam_is_overlaid_not_written(self):
    ...
    def test_08_each_attempt_uses_a_private_mount_namespace(self):
    ...
    def test_09_namespace_overlay_is_read_only_and_lifecycle_bounded(self):
        inner = section(self.script, "d287_root_namespace_attempt ()", "d287_root_series ()")
    ...
    def test_10_only_testing_mode_invokes_the_real_greeter(self):
    ...
    def test_11_wayland_and_session_bus_are_exactly_scoped(self):
        inner = section(self.script, "d287_root_namespace_attempt ()", "d287_root_series ()")
        for marker in ("/run/user/$uid", "^wayland-[0-9]+$",
                       'unix:path=$runtime/bus', "QT_QPA_PLATFORM=wayland"):
            self.assertIn(marker, inner)

    def test_12_existing_real_or_testing_greeter_is_a_gate(self):
    ...
    def test_13_d285_state_is_audited_before_and_after_every_attempt(self):
        series = section(self.script, "d287_root_series ()", "d287_operator_run ()")
    ...
    def test_14_three_slots_require_separate_operator_confirmations(self):
        series = section(self.script, "d287_root_series ()", "d287_operator_run ()")
    ...
    def test_15_match_stops_the_series_immediately(self):
        series = section(self.script, "d287_root_series ()", "d287_operator_run ()")
    ...
    def test_16_journal_boundary_is_cursor_based_and_locale_independent(self):
    ...
    def test_17_safety_telemetry_is_fail_closed(self):
    ...
    def test_18_match_fixture_requires_clean_greeter_exit(self):
    ...
    def test_19_no_match_fixture_requires_supervisor_termination(self):
    ...
    def test_20_hidden_second_epoch_is_a_safety_violation(self):
    ...
    def test_21_match_with_nonzero_greeter_exit_is_rejected(self):
        result = self._classify("result=match matched_sample=1 comparisons=1 threshold=40",
                                1, "false")
        self.assertIn("D287_01_ATTEMPT_OUTCOME=PAM_ERROR", result.stdout)
        self.assertIn("CLASSIFY_RC=1", result.stdout)

    def test_22_no_enroll_delete_restart_or_authselect_change(self):
    ...
    def test_23_capture_is_sanitized_and_excludes_template_material(self):
    ...
    def test_24_offline_preflight_does_not_call_privileged_or_live_functions(self):
    ...
    def test_25_operator_mode_is_unprivileged_and_delegates_once(self):
        operator = section(self.script, "d287_operator_run ()", "d287_offline_preflight ()")
    ...
    def test_26_readme_has_human_gate_risk_stop_and_direct_command(self):
    ...
    def test_27_methodological_review_answers_all_three_questions(self):
        for marker in ("Cosa cambia realmente", "Quale nuova ipotesi",
                       "Se fallisce nello stesso punto"):
            self.assertIn(marker, self.report)

    def test_28_upstream_provenance_and_no_code_reuse_are_explicit(self):
    ...
    def test_29_noninteractive_failure_semantics_drive_fresh_process_retries(self):
        for marker in ("non riporta lo stato aggregato a `Idle`",
                       "tre processi", "max-tries=1"):
            self.assertIn(marker, self.report)

    def test_30_result_preserves_no_live_claim(self):
    ...
    def test_31_process_group_cleanup_kills_a_term_resistant_fixture(self):
        command = (
    ...
    def test_32_match_without_greeter_unlocked_marker_is_rejected(self):
        result = self._classify("result=match matched_sample=1 comparisons=1 threshold=40",
