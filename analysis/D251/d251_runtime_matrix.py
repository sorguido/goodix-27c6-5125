...
"""D251 synthetic D4 -> exactly-one AF runtime matrix.

Run only after the D245, D246 and D251 patches have been applied in a
...
def ae_response(flags: int = 0x02, *, byte0: int = 1, size: int = 16) -> bytes:
    data = (bytes((byte0, flags)) + bytes(14))[:size].ljust(size, b"\x00")
...
class TimeoutAfterIncomingUsbApi(FakeUsbApi):
    def bulk_in(self, handle, endpoint, maximum, timeout_ms):
        if not self.incoming:
            ...
            raise UsbTimeout("D251 injected bounded timeout")
...
def run_case(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="d251-runtime-case-") as directory:
        root = Path(directory)
        facade = FakeOsFacade()
        facade.active = True
...
def run_matrix() -> dict[str, object]:
    ...
    require(report["af_state_byte0"] == 1, "state byte0 not recorded")
    ...
    require(report["af_pov_valid"] is False, "POV flag not retained")
    require(report["af_tls_connected"] is True, "TLS flag not retained")
    require(report["af_locked"] is False, "locked flag not retained")
    require(report["af_response_class"] == "valid_ae_accepted", "valid AE class")
    ...
    require(mapped["decision"] == "D251_AF_RESPONSE_VALIDATED_STOP_AFTER_AF", "decision mapping")
    require(mapped["D251_STOP_BOUNDARY"] == "STOP_AFTER_AF", "terminal mapping")
    ...
    require(timeout["report"]["af_response_class"] == "no_response", "AF timeout response class")
    ...
    require(unexpected_ack["report"]["af_response_class"] == "unexpected_ack", "AF ACK response class")
    ...
    for name, response in malformed_cases.items():
        ...
        require(case["report"]["af_response_class"] == "malformed_response", f"malformed class {name}")
        assert_terminal_counts(case, af_response_count=0)
    tests["T6_MALFORMED_AE_MATRIX"] = "PASS_LENGTH_CONTROL_CHECKSUM"

    for byte0 in (0, 2):
        unusual = run_case([*normal_frames(), D4_ACK, ae_response(byte0=byte0)])
        require(unusual["report"]["result"] == "pass", f"opaque byte0 rejected:{byte0}")
        require(unusual["report"]["af_state_byte0"] == byte0, f"byte0 lost:{byte0}")
        require(unusual["report"]["af_response_class"] == "valid_ae_with_unusual_state_fields", f"byte0 class:{byte0}")
        require(unusual["report"]["af_failure_class"] == "none", f"byte0 failure:{byte0}")
        assert_terminal_counts(unusual, af_response_count=1)
    tests["T7_OPAQUE_BYTE0_VARIANTS"] = "PASS_ACCEPTED_RETAINED_STOP_AFTER_AF"
    ...
    require(D235_RESULT_SCHEMA == "d251-live-af-single-shot-result-v1", "D251 schema")
    paths = ProductionRuntimePaths.system_default()
    require(paths.report_directory.name == "d251-results", "D251 result namespace")
    require(paths.single_use_marker.name == "d251-operator-invocation.marker", "D251 marker")

    return {
        "schema": "d251-runtime-matrix-v1",
        "status": "PASS",
        "tests": tests,
        "synthetic_af_success_count": 4,
