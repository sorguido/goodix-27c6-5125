...
| CONFIG90 224-byte length and arithmetic finalizer | `KEEP_UNIVERSAL` | Recomputed from the user's bytes |
...
| manifest, transport, CONFIG90, FDT cache and OTP digests | `MAKE_DEVICE_DYNAMIC` | Generated from and checked against each user's bundle |
...
| Packet index 103 | `REMOVE_OBSOLETE` | Never used by the semantic extractor or runtime |
...
same reader-specific evidence. A mismatched, missing, malformed, ambiguous, or
insecurely stored input remains rejected.
