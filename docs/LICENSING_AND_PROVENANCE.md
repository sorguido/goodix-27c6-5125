...
| `libfprint-driver/goodix_fpimage_device.[ch]` stage cap, `tools/d279_stage8_enroll.c` and `operator_kit/d279-57-stage8-early-terminal/` | Local integration over the authentic D279/56 aggregate and existing D279/49–52 SIGFM path | Local D279/57; no new external source import | D279/56 aggregate-only result; existing local production graph/audit APIs | Existing driver files retain `LGPL-2.1-or-later`; tool and kit are `GPL-2.0-or-later`; linked R2 combined build remains GPL-compatible | Local project authorship; prior Rockytkg/libfprint attribution remains attached to reused components | `D279/57` | `LOCAL_EVIDENCE_ADAPTATION`: stages a fixed-eight production candidate solely to test the target-local early-terminal boundary. The authentic 21-stage profile remains regression evidence. No Rockytkg wire expression or new biometric material is imported, and the future live result is not pre-claimed. |
