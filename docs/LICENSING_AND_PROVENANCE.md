...
| `production/` | file-specific; orchestration/report scripts `GPL-2.0-or-later` | Composition metadata and build support preserve their stated per-file licenses. This directory is authority for composition, not a relicensing boundary; Fedora, Rocky, local and binary dependencies keep their existing terms. |
...
| `reference/libfprint-fedora44-1.94.100/source/libfprint/{fp-image.c,fp-image-device.c,fpi-image-device.c,fpi-image.h,fpi-image-device.h,fp-image-device-private.h}` and bounded Meson delta; D279/57 parity adaptation in the corresponding four `Rockytkg/libfprint/libfprint/` files | Rockytkg materialized libfprint submodule, semantically forward-ported onto Fedora 44/libfprint 1.94.100; local R2/ownership wrappers and local terminal-completion contract | `7ebe0c809b4d1df3400e84299a4ec4acdea84590`; local D279/49–50 and D279/57 | `Rockytkg/libfprint/libfprint/{fp-image.c,fp-image-device.c,fpi-image-device.c,fpi-image.h,fpi-image-device.h,fp-image-device-private.h}` | upstream/action and SIGFM sources `LGPL-2.1-or-later`; the linked R2 component remains `GPL-2.0-or-later`, making the Goodix-enabled combined build GPL-compatible | Original libfprint/Rockytkg holders plus local adaptation authorship | `D279/52`; terminal hold `D279/57` | `ADAPTED_FROM_ROCKY`: algorithm dispatch and asynchronous SIGFM extraction are forward-ported without replacing the newer 1.94.100 state machine. Local changes add decoded session-baseline → R2 wiring, checked atomic template append, fatal error propagation and production Meson/OpenCV closure. D279/57 adds a project-local hold for successful final enrollment completion until the target-local release event; the Rockytkg-tree copy is test parity, not a retroactive upstream fact. Historical Fedora and Rockytkg provenance/license facts are unchanged. |
...
| `libfprint-driver/goodix_fdt_irq_policy.[ch]` and contextual bindings in the enrollment/legacy lifecycle | Local implementation over hash-gated APP12509 IRQ metadata and existing OEM static-analysis facts; Rockytkg independently corroborates the six-channel algorithm | D279/10 ATTEMPT02 SHA-256 `3575ca810b969f1f248684d105c679e408ef88a0c83370387e9e90d541d10eab`; local D279/59 | No Rockytkg source expression copied; behavioral comparison against `Rockytkg/src/goodix_base.c` at preserved commit `227eba219fa9e3fbac5bd59aca79f624f67cd11b` | `LGPL-2.1-or-later` | Local project authorship | `D279/59` | `LOCAL_TARGET_EVIDENCE_ADAPTATION`: centralizes exact zero-touch and bounded nonzero six-bit contact policy plus per-channel FDT-up derivation. OEM APP12509 evidence is authority; Rockytkg corroboration does not establish USB transcript identity. No protected IRQ raw bytes, biometric data or external code imported. |
| `libfprint-driver/tests/test_goodix_d278_secure_session.c`, `test_goodix_fedora44_nbis_action.c` and the SIGFM test-double score control | Existing local test infrastructure and public libfprint API composition | Local D279/28, D279/50–55 and D279/57–59 | No external expression imported; composes already-ledgered local/target Fedora paths | `LGPL-2.1-or-later` per existing files | Local project authorship | `D280/01` | `LOCAL_TEST_COMPOSITION`: proves offline close/open epoch ownership, FP3 reuse, match/no-match and corrupt-storage rejection. The production-shaped shell uses an explicit deterministic SIGFM double; real SIGFM/FP3 is exercised separately in the already-ledgered Fedora 44 target build. No production sensor-reaching file, proprietary material or biometric data is added. |
| `tools/d280_ephemeral_template_reuse.c`, `operator_kit/d280-01-ephemeral-template-reuse/` and structural test | Public libfprint API plus existing local production audit accessor and D279 operator guardrail pattern | Local D280/01 | No external expression imported; composes already-ledgered Fedora target build, local driver and POSIX/GLib public APIs | `GPL-2.0-or-later` | Local project authorship | `D280/01` | `LOCAL_OPERATOR_COMPOSITION`: two-action/two-open-epoch client with baseline/grant gates and ephemeral root-0600 FP3 host storage. It adds no production driver expression and imports no biometric data. Offline preflight only; live and protected material remain Human Gate. |
| `reference/fprintd-fedora44-1.94.5/` | Fedora 44 source RPM for the exact installed `fprintd-1.94.5-5.fc44.x86_64` | SRPM SHA-256 `886d192ad57e52d3f953e86d78ba72b389bc09d433d6ff999421542ea381f292`; upstream archive SHA-256 `a026ef34c31b25975275cc29a5e4eba2b54524769672095a5228098a08acd82c` | Full unmodified `fprintd-v1.94.5` source plus Fedora spec; spec has no patch directives | Per-file upstream terms, predominantly `GPL-2.0-or-later`; exact notices retained | Original fprintd copyright holders | `D282/01` | `EXACT_READ_ONLY_REFERENCE`: extracted without installation to audit one-print VERIFY dispatch, automatic retry and delete/storage behavior. Project production code does not link this tree. No secret or target/local-user capture/template is present; public upstream `tests/prints/` fixtures remain unmodified as source-package test data. |
| Fedora 1.94.100 VERIFY delta in `reference/libfprint-fedora44-1.94.100/source/libfprint/{fp-image-device.c,fpi-image-device.c,fpi-device.c}`, Goodix selection/audit in `libfprint-driver/goodix_fpimage_device.[ch]`, tests and D282 kit | Existing Fedora core, already-ledgered Rockytkg SIGFM matcher semantics, and local D280 target-proven single-acquisition lifecycle | Fedora source digest already recorded; Rockytkg materialized commit `7ebe0c809b4d1df3400e84299a4ec4acdea84590`; local D282/01 | Bounded action dispatch/report/completion adaptation; no new external Goodix wire implementation | Core/driver files retain `LGPL-2.1-or-later`; kit/report are `GPL-2.0-or-later`; Goodix-enabled combined build remains GPL-compatible due to R2 | Original libfprint/Rockytkg holders plus local adaptation authorship | `D282/01` | `BOUNDED_ACTION_ADAPTATION`: registers VERIFY in the existing image pipeline, matches SIGFM against the specific verify template, and shares Goodix IDENTIFY's target-proven single-acquisition graph. The production fence blocks fprintd's second formal retry before new TLS/USB. No protocol command, persistence family, PAM integration or biometric material is added. |
| Private outcome callbacks in `reference/libfprint-fedora44-1.94.100/source/libfprint/{fpi-image-device.c,fpi-image-device.h}`, consumer in `libfprint-driver/goodix_fpimage_device.c` and parity copy in `Rockytkg/libfprint/libfprint/{fpi-image-device.c,fpi-image-device.h}` | Fedora/libfprint v1.94.100 pristine plus local adaptation; the Rocky tree is a parity destination, not the source of the callbacks | Fedora pristine `f609c865f760768edb6a9e404b863ccd0569e1c8`; Rocky original identity recorded as gitlink `7ebe0c809b4d1df3400e84299a4ec4acdea84590`; local `identify_result` introduction `cb918c6114f518d10d386d2b639eb3c8e5dd5f20`; R9 corrective `c540d676ef619e3e1f1667778fd902c59566b917` | Fedora private image-device core, local Goodix consumer and Rocky test-parity copy | Core and driver files retain `LGPL-2.1-or-later`; the Goodix/R2 combined work remains under the already documented GPL-compatible regime | Original libfprint holders plus local project adaptation authorship | `D293/02`; D293 R9 corrective | `LOCAL_FEDORA_CORE_ADAPTATION_WITH_ROCKY_TEST_PARITY`: minimal `identify_result` and `verify_result` outcome notifications, absent from both pristine origins, order asynchronous host matching before APP12509 release/reopen. Clean results may enable a later explicit client action; retry/fatal/cancel remain fenced. No Rocky USB, worker, provisioning, PSK, persistence or retry expression is imported. |
...
| `libfprint-driver/goodix_enrollment_diversity.[ch]` and bounded integration in `goodix_enrollment_{model,pipeline,command_plan}.c` / `goodix_fpimage_device.c` | Rockytkg preserved enrollment selector, minimally adapted to the target-proven APP12509 lifecycle and current Fedora 44 libfprint fork | `227eba219fa9e3fbac5bd59aca79f624f67cd11b` | `Rockytkg/src/goodixgf.c` (`gf_enroll_frame_dup`, 3/8/2/MAD<8 policy and `fpi_image_device_retry_scan` semantics) | source and new helper/integration files `LGPL-2.1-or-later`; linked R2 component remains `GPL-2.0-or-later` | Rockytkg and local adaptation authorship | `D291` | `ADAPTED_FROM_ROCKY`: exact strict MAD duplicate decision, 3..8 template-stage convergence and retry-without-template-progress enter production. Target-specific additions are a hard 20-contact bound and integration with the already proven deferred release/terminal graph. Rocky USB, worker lifecycle, provisioning, PSK, reset, persistence and environment overrides are excluded. VERIFY remains one acquisition per explicit VerifyStart. |
| `production/` composition manifest, patch series and build support | Existing Fedora 44/libfprint 1.94.100 pristine Git tree, already-ledgered local Goodix/SIGFM/R2 sources, and copied local offline build templates | pristine anchor `f609c865f760768edb6a9e404b863ccd0569e1c8`; local D292/02 | `production/{source-files.tsv,downstream-paths.txt,patches/,build-support/,build*.sh,check-source.sh}` | scripts/report GPL-2.0-or-later; GUsb declaration/template and OpenCV template LGPL-2.1-or-later; composed inputs retain their existing terms | Existing holders and local project authorship as recorded per source | `D292/02` | `COMPOSITION_ONLY`: deterministic assembly from pristine Git plus a 15-file production patch and hash-pinned 30-TU/32-header local subset. No source relicensing, imported expression, licensing-boundary change, protected material, operator-kit dependency or test-area dependency. |
| `libfprint-driver/goodix_fpimage_device.[ch]` stage cap, enrollment model/event contact-boundary corrective, `tools/d279_stage8_enroll.c` and `operator_kit/d279-57-stage8-early-terminal/` | Local integration over the authentic D279/56 aggregate, target-local D279/10 lifecycle and D279/57 live results | Local D279/57–59; no new external source import | D279/56 aggregate-only result; D279/10 target lifecycle; existing local production graph/audit APIs; final sanitized live evidence under `captures/D279_57/` | Existing driver files retain `LGPL-2.1-or-later`; tool and kit are `GPL-2.0-or-later`; linked R2 combined build remains GPL-compatible | Local project authorship; prior Rockytkg/libfprint attribution remains attached to reused components | `D279/57`; flags corrective `D279/59` | `LOCAL_EVIDENCE_ADAPTATION`: fixed-eight production policy plus local corrective separating primary-sample provenance from physical release-ready status. All eight images are delivered at release-ready; the final successful completion is held until terminal IRQ, without holding errors/cancellation. The authentic 21-stage wire profile remains regression evidence. Three failed live grants through `4de9c34…` and the final successful `38962cc…` grant are consumed; the final hash-pinned run validates eight stages, seven re-arms and the D279/59 contextual flag policy on APP12509. No Rockytkg wire expression or biometric material is imported. |
