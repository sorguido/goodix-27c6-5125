...
| `libfprint-driver/goodix_fpimage_device.[ch]` stage cap, enrollment model/event contact-boundary corrective, `tools/d279_stage8_enroll.c` and `operator_kit/d279-57-stage8-early-terminal/` | Local integration over the authentic D279/56 aggregate, target-local D279/10 lifecycle and D279/57 operator-declared live result | Local D279/57; no new external source import | D279/56 aggregate-only result; D279/10 target lifecycle; existing local production graph/audit APIs | Existing driver files retain `LGPL-2.1-or-later`; tool and kit are `GPL-2.0-or-later`; linked R2 combined build remains GPL-compatible | Local project authorship; prior Rockytkg/libfprint attribution remains attached to reused components | `D279/57` | `LOCAL_EVIDENCE_ADAPTATION`: fixed-eight production candidate plus local corrective separating primary-sample provenance from physical release-ready status. All eight images are delivered at release-ready; the final successful completion is held until terminal IRQ, without holding errors/cancellation. The authentic 21-stage wire profile remains regression evidence. Attempt 01 on `1afe72e…` failed closed after 2/8 stages; its original root-only files remain pending safe import and are not pre-claimed. No Rockytkg wire expression or biometric material is imported. |
