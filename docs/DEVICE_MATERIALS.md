...
The implementation uses the documented SHA-256/HMAC/AES derivation and is
...
The final staging directory used by the public installer must contain exactly:
...
```bash
find "$HOME/goodix-5125-materials" -maxdepth 1 -type f -printf '%f %s bytes\n' | sort
sha256sum "$HOME/goodix-5125-materials/"*
```
...
# Install with the completed bundle

Once the five-file bundle is complete in:
...
`d232-target-material-v1` manifest. The final manifest produced in Step 10 is
already the canonical `goodix-5125-device-materials-v1` form expected by the
public installer.
