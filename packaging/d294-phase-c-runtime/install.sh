...
if [[ ${1:-} == --root-install ]]; then
  ...
  [[ -f $rpm_path && ! -L $rpm_path &&
     $(sha256sum "$rpm_path" | awk '{print $1}') == "$expected_sha" ]] ||
    fail rpm_drift
  ...
  flann_before=false
  rpm -q opencv-features2d >/dev/null 2>&1 && features_before=true
  rpm -q opencv-flann >/dev/null 2>&1 && flann_before=true
  mutated=false
  cleanup_failed_install() {
    ...
    if [[ $mutated == true && $rc -ne 0 ]]; then
      ...
      if rpm -q "$package" >/dev/null 2>&1; then
        dnf5 remove -y "$package" >/dev/null 2>&1 || true
      fi
      if [[ $features_before == false ]] && rpm -q opencv-features2d >/dev/null 2>&1; then
        dnf5 remove -y opencv-features2d >/dev/null 2>&1 || true
      fi
      if [[ $flann_before == false ]] && rpm -q opencv-flann >/dev/null 2>&1; then
        dnf5 remove -y opencv-flann >/dev/null 2>&1 || true
      fi
