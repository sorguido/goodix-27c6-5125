...
sono PASS. Le suite integrate D291 diversity (4/4), secure-session (34/34) e
...
Il riesame R9 ha poi confermato un'incompatibilità con la normale serie PAM
multi-dito: fprintd mantiene Claim/open e ogni nuovo `VerifyStart("any")` su
gallery multipla richiede una nuova IDENTIFY, ma il vecchio ramo dopo la prima
IDENTIFY pulita ammetteva soltanto ENROLL. La regressione sul vero adattatore ha
riprodotto il rifiuto; il commit
`c540d676ef619e3e1f1667778fd902c59566b917` aggiunge il rollover
fresh-resource per la sola nuova IDENTIFY esplicita. NO_MATCH→MATCH e tre
NO_MATCH nello stesso open sono PASS normal/sanitizer senza quarta acquisizione.
Processing retry/fatal, cancellazione, non-drain e poison restano fenced senza
reacquire; il fatal matcher è attraversato dinamicamente fino al poison e al
rifiuto della successiva IDENTIFY senza nuova transport epoch.

I callback privati outcome-only `identify_result` e `verify_result` ordinano il
confronto SIGFM asincrono prima del rilascio: STOP/drain può precedere il
matcher, mentre un `FP_DEVICE_RETRY` fprintd non è una nuova richiesta client.
Sono adattamenti locali assenti da Fedora pristine e Rocky originale; la copia
Rocky corrente serve solo alla parità dei test. Non aggiungono simboli pubblici
né importano worker, USB, provisioning o retry Rocky. Restano invariati STOP,
drain, fresh acquire, sticky poison e continuità della baseline APP12509.
...
offline: dito fisico non già registrato e duplicato distinto; nuovo account
senza accesso al journal; marker root-collected whitelistati e capture
self-contained; prompt streaming; chiusura del KCM prima di VERIFY e riapertura
per delete; rollback serializzato, recovery attribuita/idempotente, unmount e
template fail-closed. Conteggi `fprintd-list` e campi audit richiedono forma,
cardinalità e un solo `action=` per record; durante una finestra post-action
...
`docs/libfprint-integration.md`, `libfprint/libfprint/sigfm/sigfm.cpp` e le
integrazioni libfprint dello snapshot. L'esito per l'ordine obbligatorio è:

1. **lifecycle:** nessun delta. Il teardown/reopen locale è richiesto dalle
   sequenze APP12509 osservate e ha già provato una acquisition per
   `VerifyStart`; il worker/device lifetime Rocky non può sostituirlo senza
   importare assunzioni transport e recovery non provate sul target;
2. **baseline:** nessun delta. La B0 locale è un input di normalizzazione
   target-observed per sessione, mentre `img_base` Rocky include anche una
   calibrazione di device ricostruita sulla deriva e potenzialmente persistita.
   Una equivalenza semantica forte non è provata;
3. **preprocessing:** nessun delta. Il production R2 è già l'adattamento
   minimo di `gx_imgproc_to8bit()` con `GX_IMGPROC_SIGFM_PARAMS`: sottrazione
   baseline, correzione low-frequency/flat-field, percentile stretch e
   unsharp, sull'esatto raster 80x64 U8 passato a SIGFM;
4. **enrollment diversity:** era la divergenza materiale senza ragione
   target-specific. La production accettava otto pressioni senza distinguere
   contatti fisici e sample utili, mentre Rocky seleziona copertura spaziale;
5. **retry:** un duplicato enrollment è una scansione retryable della stessa
   operazione; un'immagine VERIFY valida con NO_MATCH termina invece la
   `VerifyStart` e lascia al solo consumer/PAM l'eventuale tentativo seguente;
6. **threshold:** resta 40. Abbassarlo a 20 non trasformerebbe gli score zero
   in match e non è supportato da un corpus same-finger/different-finger.

La divergenza 4 è ora corretta in production con
`goodix_enrollment_diversity.[ch]`, adattamento minimo della funzione Rocky
`gf_enroll_frame_dup()`: minimo 3 e massimo 8 sample, duplicato quando il MAD
U8 è strettamente minore di 8, convergenza dopo due duplicati consecutivi. Un
duplicato non terminale chiama `fpi_image_device_retry_scan()` e non avanza il
template; il contatto di convergenza diventa lo stage finale, come in Rocky.
Il modello/protocol plan mantiene separati contatti osservati e stage
consegnati, aggiorna dinamicamente `nr_enroll_stages` e conserva il terminal
hold APP12509 già validato. Un limite locale aggiuntivo di 20 contatti rende
il percorso bounded e fail-closed.
...
Dettaglio del riesame e del delta: `analysis/D291/D291_01_OFFLINE_CLOSURE.md`.
...
contratto statico del KCM Users installato. Il correttivo D293 R9 ha inoltre
chiuso il difetto di lifecycle che impediva IDENTIFY ripetute nello stesso
...
Attempt 04 e Attempt 05 hanno poi eseguito la baseline
`ed94d33e1cf6a2387135598c58a27d6c1573ba13`. Entrambe le run software hanno
completato enrollment, restart, due VERIFY, delete ed export: quattro epoch,
tre action consumate, tre handshake e zero retry/reopen/reset/clear-halt o
famiglie persistenti note; tutti i backend sono drenati, i context chiusi e
servizio, staging, libreria e storage preesistente ripristinati.

Le due run restano semanticamente distinte. In Attempt 04 l'operatore attesta
di avere presentato l'indice destro anche in Phase B: quella run non prova il
different-finger e osserva invece, su un singolo secondo campione dello stesso
dito, un false non-match dopo un primo match. Questa osservazione non stima
FRR/FAR e non autorizza generalizzazioni sull'affidabilità del matcher. In
Attempt 05 l'operatore attesta otto contatti dell'indice destro per enrollment,
lo stesso dito per il match e l'indice sinistro per il no-match. Attempt 05
chiude quindi il requisito D282 different-finger, delete e rollback.

Gli `operator.log` sono differenti ma i due `summary.env` sono byte-identici:
il summary descrive risultati software, non il dito fisico. I quattro digest,
summary, output client e blocchi audit sono verificati dallo script
`analysis/D282/d282_01_attempt_04_05_evidence_audit.py`. I summary autentici
duplicano due chiavi di safety con lo stesso valore; il correttivo del launcher
le emette ora una volta sola.

L'errore di stimulus Attempt 04 è stato favorito dalla stringa client
`Verifying: right-index-finger`, che indica il template registrato e non il
...
FPRINTD_TARGET_SIGFM_STORAGE_PROVEN=true
...
D282_01_OUTCOME=PASS_LIVE_CLOSED_BY_ATTEMPT_05
D282_01_OFFLINE_CLOSURE=PASS_WITH_HASH_PINNED_LIVE_REVIEW
...
D282_01_FEDORA44_SIGFM_VERIFY_MATCH=PASS_LIVE
D282_01_DIFFERENT_FINGER_NO_MATCH=PASS_LIVE_OPERATOR_ATTESTED_STIMULUS
D282_01_FPRINTD_END_USER_CLI_INTEGRATION_PROVEN=true
D282_01_FPRINTD_RETRY_SECOND_SENSOR_REACHING_ACTION_COUNT=0
D282_01_REQUIRED_MATRIX=68/68_PASS
...
D282_01_ATTEMPT_04_BASELINE_SHA=ed94d33e1cf6a2387135598c58a27d6c1573ba13
D282_01_ATTEMPT_04_PHYSICAL_SEQUENCE=10X_RIGHT_INDEX_OPERATOR_ATTESTED
...
D282_01_ATTEMPT_04_MATCHER_RELIABILITY_GENERALIZATION_ALLOWED=false
D282_01_ATTEMPT_04_DELETE_REACHED=true
D282_01_ATTEMPT_04_ROLLBACK_COMPLETE=true
D282_01_ATTEMPT_05=PASS_LIVE_CLOSED
D282_01_ATTEMPT_05_BASELINE_SHA=ed94d33e1cf6a2387135598c58a27d6c1573ba13
D282_01_ATTEMPT_05_PHYSICAL_SEQUENCE=9X_RIGHT_INDEX_PLUS_1X_LEFT_INDEX_OPERATOR_ATTESTED
D282_01_ATTEMPT_05_SAME_FINGER_RIGHT_INDEX_VERIFY=MATCH
D282_01_ATTEMPT_05_DIFFERENT_FINGER_LEFT_INDEX_VERIFY=NO_MATCH_OPERATOR_ATTESTED_STIMULUS
D282_01_ATTEMPT_05_DELETE_REACHED=true
D282_01_ATTEMPT_05_ROLLBACK_COMPLETE=true
D282_01_ATTEMPT_04_05_SUMMARY_DUPLICATE_KEYS=AUTHORIZATION_CREDENTIAL_REQUIRED,AUTOMATIC_OR_IMPLICIT_SENSOR_RETRY_ALLOWED
D282_01_CURRENT_SUMMARY_DUPLICATE_KEY_CORRECTIVE=PASS_OFFLINE
D282_01_PHASE_B_PHYSICAL_FINGER_POKA_YOKE=PASS_OFFLINE
...
D282_02_DECISION=PASS_LIVE_CLOSED
D282_02_OUTCOME=PASS_LIVE_CLOSED_BY_ATTEMPT_02
D282_02_SECOND_VERIFY_DETERMINISTIC_NO_MATCH=NOT_REPRODUCED
D282_02_RESTART_GLOBAL_DEGRADATION=NOT_OBSERVED
D282_02_SAME_FINGER_FALSE_NON_MATCH_REPRODUCED=true
...
D282_02_LIVE_READINESS=CLOSED_DO_NOT_RERUN
D282_02_LIVE_ENTRYPOINT_FAIL_CLOSED=true
...
D282_02_ATTEMPT_02=PASS_LIVE_CLOSED
D282_02_ATTEMPT_02_BASELINE_SHA=f868ad127b30467f4d0da9aedcaef6faea8b0caf
D282_02_ATTEMPT_02_EVIDENCE_AUDIT=PASS_HASH_PINNED
D282_02_ATTEMPT_02_PHYSICAL_SEQUENCE=12X_RIGHT_INDEX_OPERATOR_ATTESTED
D282_02_ATTEMPT_02_TRIAL_RESULTS=MATCH,MATCH,MATCH,NO_MATCH
D282_02_ATTEMPT_02_TRIAL_OBSERVED_MAX_SCORES=941,129,2739,1
D282_02_ATTEMPT_02_MATCH_SCORE_VECTORS_CENSORED_BY_EARLY_RETURN=true
D282_02_ATTEMPT_02_FALSE_NON_MATCH_FULL_TEMPLATE_COVERAGE=true
D282_02_ATTEMPT_02_FRR_OR_FAR_GENERALIZATION_ALLOWED=false
D282_02_ATTEMPT_02_ACTION_ATTEMPT_COUNT=5
D282_02_ATTEMPT_02_REAL_SUBMIT_COUNT=528
D282_02_ATTEMPT_02_RETRY_COUNT=0
D282_02_ATTEMPT_02_REOPEN_COUNT=0
D282_02_ATTEMPT_02_RESET_COUNT=0
D282_02_ATTEMPT_02_CLEAR_HALT_COUNT=0
D282_02_ATTEMPT_02_KNOWN_PERSISTENT_FAMILY_COUNT=0
D282_02_ATTEMPT_02_ROLLBACK_COMPLETE=true
D282_02_ATTEMPT_02_OPERATOR_LOG_SHA256=8152304946f7d5d3ab4d8859d80970331c4505fb931644251fd320ba9eda3afb
D282_02_ATTEMPT_02_SUMMARY_ENV_SHA256=a1f84c32d23283966bf68e9f3944e676faf5e2b638f0fae78e7bb2e782cf028f
D282_02_ATTEMPT_02_TRIALS_TSV_SHA256=ba115e1772e3d31d90a98bbd103526ee2fd377558664e4290cb7a72ee9ce87ab
D282_02_ATTEMPT_02_TERMINAL_TRANSCRIPT_SHA256=cacc131befbd20ff5cbce56ad7dbf1d2a37eac28eb8ac42e22ab5112a2be69f5
D282_02_AUTONOMOUS_RERUN_ALLOWED=false
D282_03_DECISION=PASS_LIVE_CLOSED
D282_03_OUTCOME=PASS_LIVE_CLOSED_BY_ATTEMPT_02
D282_03_EXPERIMENT=BALANCED_SAME_DIFFERENT_SIX_TRIALS
D282_03_SEQUENCE=RIGHT_LEFT_RIGHT_THEN_LEFT_RIGHT_LEFT
D282_03_ENROLLMENT_CONTACT_COUNT=8
D282_03_VERIFY_TRIAL_COUNT=6
D282_03_SAME_FINGER_TRIAL_COUNT=3
D282_03_DIFFERENT_FINGER_TRIAL_COUNT=3
D282_03_BLOCK_COUNT=2
D282_03_TRIALS_PER_BLOCK=3
D282_03_DAEMON_RESTART_COUNT=2
D282_03_SIGFM_THRESHOLD_EXPECTED=40
D282_03_ALGORITHM_CHANGE=false
D282_03_ENROLLMENT_STAGE_CHANGE=false
D282_03_PREPROCESSING_CHANGE=false
D282_03_BIOMETRIC_ACTION_MAX=7
D282_03_EXPECTED_PHYSICAL_CONTACT_COUNT_MAX=14
D282_03_AUTOMATIC_OR_IMPLICIT_SENSOR_RETRY_ALLOWED=false
D282_03_OFFLINE_CONTRACT_MATRIX=17/17_PASS
D282_03_COMBINED_REGRESSION_MATRIX=118/118_PASS
D282_03_CANDIDATE_BUILD=PASS_OFFLINE
D282_03_FPRINTD_ABI_CLOSURE=PASS_OFFLINE
D282_03_OPERATOR_KIT=operator_kit/d282-03-balanced-same-different
D282_03_EXECUTABLE_CLOSURE=PASS_OFFLINE
D282_03_LIVE_READINESS=CLOSED_DO_NOT_RERUN
D282_03_LIVE_ENTRYPOINT_FAIL_CLOSED=true
D282_03_ATTEMPT_02=PASS_LIVE_CLOSED
D282_03_ATTEMPT_02_BASELINE_SHA=4a3ee4bb96f6f4e9bca838aabb51edc9ce4b2db1
D282_03_ATTEMPT_02_EVIDENCE_AUDIT=PASS_HASH_PINNED
D282_03_ATTEMPT_02_EVIDENCE_CANONICAL=true
D282_03_ATTEMPT_01_EVIDENCE_CANONICAL=false
D282_03_ATTEMPT_02_PHYSICAL_SEQUENCE=8X_RIGHT_INDEX_THEN_R_L_R_L_R_L_OPERATOR_CONFIRMED
D282_03_ATTEMPT_02_SAME_FINGER_RESULT=3_OF_3_MATCH
D282_03_ATTEMPT_02_DIFFERENT_FINGER_RESULT=3_OF_3_NO_MATCH
D282_03_ATTEMPT_02_SAME_FINGER_OBSERVED_MAX_SCORES=674,75,54
D282_03_ATTEMPT_02_DIFFERENT_FINGER_OBSERVED_MAX_SCORES=0,0,0
D282_03_ATTEMPT_02_SAME_FINGER_MATCHED_SAMPLES=1,1,5
D282_03_ATTEMPT_02_MATCH_SCORE_VECTORS_CENSORED_BY_EARLY_RETURN=true
D282_03_ATTEMPT_02_DIFFERENT_FINGER_SCORE_VECTORS_COMPLETE=true
D282_03_ATTEMPT_02_SAME_FINGER_PROBE_KEYPOINTS=155,148,139
D282_03_ATTEMPT_02_DIFFERENT_FINGER_PROBE_KEYPOINTS=115,117,111
D282_03_ATTEMPT_02_ACTION_ATTEMPT_COUNT=7
D282_03_ATTEMPT_02_REAL_SUBMIT_COUNT=678
D282_03_ATTEMPT_02_RETRY_COUNT=0
D282_03_ATTEMPT_02_REOPEN_COUNT=0
D282_03_ATTEMPT_02_RESET_COUNT=0
D282_03_ATTEMPT_02_CLEAR_HALT_COUNT=0
D282_03_ATTEMPT_02_KNOWN_PERSISTENT_FAMILY_COUNT=0
D282_03_ATTEMPT_02_ROLLBACK_COMPLETE=true
D282_03_ATTEMPT_02_OPERATOR_LOG_SHA256=0877ef2aa822107667a5161ce2aadb701a9f46cdf6604e7dd0ffe07674718916
D282_03_ATTEMPT_02_SUMMARY_ENV_SHA256=5186b8bef974f95b661394915c9b1ea78f895be6ad3fa60156fcb571f71d8394
D282_03_ATTEMPT_02_TRIALS_TSV_SHA256=0eac5367ef03f2b0dbfcd949f82b9ca927ddbb9ddda1e3617100f1b2116bd026
...
D283_01_ACTUAL_CANDIDATE_SYMLINK_STAGING_REGRESSION=PASS
D283_01_PRE_ACTION_RESULT_EXPORT_REGRESSION=PASS
D283_01_SIGFM_MATCH_DIAGNOSTICS_REQUIRED=true
D283_01_ENROLLMENT_PHYSICAL_FINGER_POKA_YOKE=PASS_OFFLINE
...
`analysis/D282/D282_02_attempt_02_post_live_review.md`,
`analysis/D282/D282_02_ATTEMPT_02_NORMALIZED.env`,
`captures/D282_02/D28202_ATTEMPT_02_20260910T220312Z_f868ad127b30/sanitized/`,
`analysis/D282/D282_03_balanced_same_different_boundary.md`,
`analysis/D282/D282_03_OFFLINE_RESULT.env`,
`analysis/D282/d282_03_trial_audit.py`,
`operator_kit/d282-03-balanced-same-different/`,
`analysis/D282/D282_03_attempt_02_post_live_review.md`,
`analysis/D282/D282_03_ATTEMPT_02_NORMALIZED.env`,
...
`captures/D282_01/D28201_ATTEMPT_04_20260910T202549Z_ed94d33e1/`,
`captures/D282_01/D28201_ATTEMPT_05_20260910T203511Z_ed94d33e1/`,
`analysis/D282/D282_01_attempt_03_post_live_analysis.md`,
...
D279/48 ha ora chiuso offline l'esperimento discriminante, senza modificare il
percorso production. L'audit del codice preservato conferma che Rockytkg usa
una vera no-finger baseline 80×64 e tratta ogni pressione 80×64 come campione
autonomo di un template multi-sample, senza resize o image fusion. La pipeline
comune è `clamp(2048+raw-baseline)` → flat-field r12 → percentile 1/99; il
solo stadio SIGFM-specifico è unsharp boost 0,8/sigma 1,5.

Il comparator compila direttamente i sorgenti Rockytkg/SIGFM preservati e il
NBIS/Bozorth Fedora 44 libfprint 1.94.100. R1 e R2 consegnano lo stesso identico
raster nativo 80×64 a entrambi gli extractor e aggregano feature, gate e score
same-session nelle due direzioni, senza esportare dati biometrici per-frame.
Ventisei test, build dei tre helper, KAT, seam audit ed end-to-end sintetico
sono verdi; preprocessor e NBIS passano anche ASan/UBSan. Nessuna seam
host-only è raggiungibile dal percorso USB production. Il limite temporale del
kit è un safety bound host-side e non implica quiescenza device-side.

ATTEMPT02 resta un solo contesto sessione/dito, senza negative o
different-finger control; non consente FAR/FRR, accuratezza o validazione di
soglie production. Il suo B0 resta una baseline sperimentale, non una prova di
equivalenza con la vera no-finger baseline Rocky. D279/02 resta strutturalmente
NBIS ma biometricamente sotto review. Il prossimo confine è il Human Gate su
SHA completo per una sola lettura protected offline D279/48, zero USB/live e
zero retry. Nessuna autorizzazione è attualmente attiva.
...
CURRENT_LIVE_AUTHORIZED=false
HISTORICAL_NEXT_PRIMARY_BOUNDARY_AFTER_D279_28=HUMAN_GATE_EXACT_SHA_FOR_D279_29_ONE_SHOT_ENROLLMENT
```
...
### D279/40–D279/48 — quality review, matrice half-wave e confronto biometrico
...
D279/48 compila poi la pipeline Rockytkg esatta e costruisce il confronto
same-input R1/R2 tra NBIS e SIGFM, con score aggregati bidirezionali e kit
one-shot protetto. La decisione A/B/C/D resta sospesa fino all'evidenza
autentica; production, sender e allowlist non cambiano.
...
`analysis/D279/D279_47_authentic_halfwave_polarity_review.md` e
`analysis/D279/D279_48_rocky_nbis_sigfm_comparison_boundary.md`.
