...
build_dir=$(mktemp -d /tmp/goodix-d27403-aggr.XXXXXX)
...
gcc $strict_flags $glib_cflags $includes -c \
  "$git_root/libfprint-driver/goodix_capture_aggregation.c" \
  -o "$build_dir/goodix_capture_aggregation.o"

for obj in goodix_u16_to_fpimage goodix_fpimage_pipeline goodix_capture_aggregation; do
  if nm -u "$build_dir/$obj.o" | \
     grep -E "(^|[[:space:]])(libusb_|usb_|fopen|open|read|write|socket|SSL_|mbedtls_|gnutls_)"; then
    echo "forbidden I/O, USB, or TLS symbol in production helper: $obj" >&2
    exit 1
  fi
done
echo "goodix_capture_aggregation forbidden-symbol audit: PASS"

gcc $strict_flags $glib_cflags $includes -c \
  "$test_dir/test_goodix_capture_aggregation.c" \
  -o "$build_dir/test_goodix_capture_aggregation.o"
...
gcc -Wl,--gc-sections \
  ...
  "$build_dir/goodix_capture_aggregation.o" \
  "$build_dir/test_goodix_capture_aggregation.o" \
  "$build_dir/fp-image-local.o" \
  "$build_dir/fpimage_link_stubs.o" \
  $glib_libs -o "$build_dir/test_goodix_capture_aggregation"
"$build_dir/test_goodix_capture_aggregation"
...
gcc $san_strict_flags $glib_cflags $includes -c \
  "$git_root/libfprint-driver/goodix_capture_aggregation.c" \
  -o "$build_dir/goodix_capture_aggregation_san.o"
gcc $san_strict_flags $glib_cflags $includes -c \
  "$test_dir/test_goodix_capture_aggregation.c" \
  -o "$build_dir/test_goodix_capture_aggregation_san.o"
...
gcc $san_common -Wl,--gc-sections \
  ...
  "$build_dir/goodix_capture_aggregation_san.o" \
  "$build_dir/test_goodix_capture_aggregation_san.o" \
  "$build_dir/fp-image-local_san.o" \
  "$build_dir/fpimage_link_stubs_san.o" \
  $glib_libs -o "$build_dir/test_goodix_capture_aggregation_sanitized"
ASAN_OPTIONS=detect_leaks=0:halt_on_error=1 \
UBSAN_OPTIONS=halt_on_error=1 \
  "$build_dir/test_goodix_capture_aggregation_sanitized"
' d27403-aggr-test "$git_root" "$build_dir"
