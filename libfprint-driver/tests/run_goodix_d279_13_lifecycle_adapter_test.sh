...
if [ "${GOODIX_D279_13_IN_SDK:-0}" != 1 ] &&
  ...
  exec flatpak run --user --unshare=network \
    ...
    --env=GOODIX_D279_13_IN_SDK=1 \
    --command=sh org.freedesktop.Sdk//25.08 "$0"
fi

build=$(mktemp -d /tmp/goodix-d279-13-adapter.XXXXXX)
...
if grep -En '(goodix_fpi_usb|goodix_a0_build|fixed64_command|submit_out|g_usb_|libusb_)' \
    "$root/libfprint-driver/goodix_enrollment_lifecycle_adapter.c" \
    "$root/libfprint-driver/goodix_enrollment_lifecycle_adapter.h"; then
  echo "wire serializer or sender dependency in D279/13 adapter" >&2
  exit 1
fi
echo D279_13_ZERO_SENDER_SOURCE_AUDIT=PASS
...
run_build () {
  ...
  # shellcheck disable=SC2086
  gcc $strict $extra $cflags $includes -c \
    "$script_dir/test_goodix_enrollment_lifecycle_adapter.c" \
  ...
  if nm -u "$build/goodix_enrollment_lifecycle_adapter_${suffix}.o" | \
     grep -E '(^|[[:space:]])(g_usb_|libusb_|SSL_|mbedtls_|gnutls_|open|read|write|socket)'; then
    echo "forbidden I/O, USB or TLS symbol in D279/13 adapter" >&2
    exit 1
  fi
  # shellcheck disable=SC2086
  gcc $extra "$build"/*_"$suffix".o $libs -lm \
    -o "$build/adapter_${suffix}"
  ASAN_OPTIONS=detect_leaks=0:halt_on_error=1 \
  UBSAN_OPTIONS=halt_on_error=1 timeout 30 "$build/adapter_${suffix}"
}

run_build normal ""
echo D279_13_NORMAL=PASS
run_build sanitized "-O1 -fno-omit-frame-pointer -fsanitize=address,undefined"
echo D279_13_ASAN_UBSAN=PASS
...
echo PREPARE_CACHE_TIMESTAMP_STABILITY=PASS
