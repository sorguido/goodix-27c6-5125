...
# This suite links the repository compile stub and never instantiates GUsb;
# requiring host gusb pkg-config metadata would not strengthen the host-only
# proof and makes the SDK path needlessly environment-dependent.
...
build_run () {
  ...
  ASAN_OPTIONS=detect_leaks=0:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 timeout 60 "$build/$name"
}
build_run normal ""
...
echo CANCEL_REQUEST_DOES_NOT_RELEASE_IN_TOKEN=true
echo CANCEL_REQUEST_DOES_NOT_RELEASE_OUT_TOKEN=true
echo MATCHING_CANCELLED_CALLBACK_RELEASES_OWN_TOKEN=true
echo BEGIN_GENERATION_WHILE_NOT_DRAINED=REJECTED
echo BEGIN_GENERATION_AFTER_DRAIN=ALLOWED
echo BACKEND_FREE_BEFORE_DRAIN=PREVENTED
echo HOST_ASYNC_IO_DRAIN=PROVEN_HOST_ONLY
