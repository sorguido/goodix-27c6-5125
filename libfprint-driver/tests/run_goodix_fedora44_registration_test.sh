...
build_root=$(mktemp -d /tmp/goodix-d279-03-registration.XXXXXX)
build_dir="$build_root/build"
pkgconfig_dir="$build_root/pkgconfig"
inner="$script_dir/run_goodix_fedora44_registration_test_inner.sh"
...
cleanup ()
{
  if [ "${KEEP_BUILD:-0}" = 1 ]; then
    echo "D279_03_BUILD_ROOT=$build_root"
    return
  fi
