 ...
 * dependency with this pure host-image view and removes environment/debug
 * configuration. The image algorithm and fixed R1/R2 parameters are retained.
 */
struct gx_imgproc_frame {
    uint32_t width;
    uint32_t height;
    const uint8_t *baseline16le;
...
/* SIGFM 驱动路径固定参数：开启反锐化局部对比增强。小图（80x64）弱按压下
 ...
 * 此 production adaptation 不读取环境变量、不写 dump e non espone I/O. */
