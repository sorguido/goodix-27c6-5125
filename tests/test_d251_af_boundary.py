...
def ae(flags: int = 2, *, byte0: int = 1) -> bytes:
    return outer(payload(0xAE, bytes((byte0, flags)) + bytes(14)))
...
class D251CoreBoundaryTests(unittest.TestCase):
    def test_happy_is_terminal_and_preserves_unknown_bits(self):
        ...
        self.assertEqual((state.byte0, state.flags), (1, 0x32))
    ...
    def test_opaque_state_byte0_is_retained_without_semantic_rejection(self):
        machine = ExactlyOneAfMachine(ScriptedTransport([ae(byte0=2)]))
        state = machine.run(4)
        self.assertEqual(machine.phase, STOP_AFTER_AF)
        self.assertEqual(state.byte0, 2)
        self.assertEqual(
            (machine.af_attempt_count, machine.af_send_count, machine.af_response_count),
            (1, 1, 1),
        )
