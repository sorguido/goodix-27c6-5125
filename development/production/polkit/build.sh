...
gcc -std=gnu11 -fPIC -shared -O2 -Wall -Wextra -Werror -Wl,--no-undefined \
  ...
  -I"$output/headers/usr/include" "$here/pam_goodix_polkit.c" \
  /usr/lib64/libpam.so.0 -o "$output/pam_goodix_polkit.so"
nm -D --defined-only "$output/pam_goodix_polkit.so" >"$output/symbols"
[[ $(awk '{print $3}' "$output/symbols") == pam_sm_authenticate ]]
readelf -d "$output/pam_goodix_polkit.so" >"$output/dynamic"
! grep -E 'RPATH|RUNPATH' "$output/dynamic"
! strings "$output/pam_goodix_polkit.so" | grep -F GOODIX_POLKIT_TEST_
echo POLKIT_PAM_BUILD=PASS
