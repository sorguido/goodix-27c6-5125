...
class TransactionTest(unittest.TestCase):
    def setUp(self):
        ...
        self.temp = tempfile.TemporaryDirectory(prefix='goodix-early-login-unit-')
        ...
        (self.candidate / 'PROVENANCE').write_text('TEST_ONLY\nMODE=normal\n')
        self.m.p(self.m.GREETER_DROPIN).parent.parent.mkdir(parents=True, exist_ok=True)
        (self.candidate / 'SHA256SUMS').write_text(''.join(
            f'{self.m.digest((self.candidate / name).read_bytes())}  {name}\n' for name in self.m.FILES))

    def run_fake(self, *args):
        self.calls.append(args)
        if args[0] in ('restorecon', 'chcon'):
    ...
    def assert_original(self):
        self.m.baseline()
        for name in (self.m.PAM, self.m.WRAPPER, self.m.DROPIN, self.m.GREETER_DROPIN, self.m.RUNTIME, self.m.STATE):
    ...
    def test_install_rollback_idempotent(self):
        ...
        self.assertIn(b'max-tries=1 timeout=8', self.m.read(self.m.PAM))
