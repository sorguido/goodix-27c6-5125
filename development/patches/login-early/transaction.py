...
"""One reversible early-login overlay on the observed D293 deployment; no material access."""
import hashlib
import json
import os
...
RUNTIME = '/usr/local/lib64/goodix-27c6-5125/login-early'
WRAPPER = '/usr/local/sbin/goodix-login-early-fprintd'
DROPIN = '/etc/systemd/system/fprintd.service.d/96-goodix-login-early.conf'
PAM = '/etc/pam.d/plasmalogin'
STATE = '/etc/goodix-27c6-5125/login-early.json'
...
FILES = (*LIBS, 'fprintd', 'greeter', 'pam_fprintd.so', 'PROVENANCE', 'source-files.sha256')
GREETER_DROPIN = '/etc/systemd/user/plasma-login.service.d/96-goodix-login-early.conf'
GREETER_UNIT = '/usr/lib/systemd/user/plasma-login.service'
BASE_RUNTIME = '/usr/local/lib64/goodix-27c6-5125/d293-native-e61fce313794922a2dab156a1b38a8ddc5837f19'
BASELINE = {
    '/usr/libexec/plasma-login-greeter': 'b40c3c90d1de811ffcb41b45afd4d485e1f0c478d173607b2b48c03c540e7ac0',
    '/usr/bin/plasmalogin': '4de1dfb9181818c43e6a4481d3c1ba2272ddac1218f727418301e45bffc54ebd',
    GREETER_UNIT: '97d00cf76a68583453ecee781c2bc8e9b2411d842464290698a2ad978f70f8f0',
...
def write(name, data, mode=0o644):
    ...
    fd, temp = tempfile.mkstemp(prefix='.early-login-', dir=dest.parent)
    try:
...
def validate_owned(state, partial=False):
    expected = {RUNTIME + '/' + x for x in (*FILES, 'SHA256SUMS')} | {PAM, WRAPPER, DROPIN, GREETER_DROPIN}
...
def remove_owned(state):
    ...
    if p(RUNTIME).exists():
        p(RUNTIME).rmdir()
    if state['greeter_dir_created'] and p(GREETER_DROPIN).parent.exists():
        p(GREETER_DROPIN).parent.rmdir()


def rollback():
    if not p(STATE).exists():
        require(not any(p(x).exists() or p(x).is_symlink() for x in (RUNTIME, WRAPPER, DROPIN, GREETER_DROPIN)),
                'overlay exists without rollback state')
        print('EARLY_LOGIN_ROLLBACK=ALREADY_ABSENT')
        return
    ...
    p(STATE).unlink()
    print('EARLY_LOGIN_ROLLBACK=PASS previous=D293 PAM=vendor')
...
def install(candidate):
    ...
    if p(STATE).exists():
        ...
        effective(WRAPPER)
        print('EARLY_LOGIN_INSTALL=ALREADY_ACTIVE')
        return
    effective(PREVIOUS)
    for name in (RUNTIME, WRAPPER, DROPIN, GREETER_DROPIN, PAM, STATE):
        require(not p(name).exists() and not p(name).is_symlink(), f'expected absent: {name}')
    for directory in ('/etc/systemd/user', '/usr/local/lib/systemd/user',
                      '/usr/lib/systemd/user', '/run/systemd/user',
                      '/var/lib/plasmalogin/.config/systemd/user'):
        unit = p(directory + '/plasma-login.service')
        if str(unit) != str(p(GREETER_UNIT)):
            require(not unit.exists() and not unit.is_symlink(), 'greeter unit override present')
        dropins = p(directory + '/plasma-login.service.d')
        require(not dropins.is_symlink(), 'greeter drop-in directory is a symlink')
        require(not dropins.exists() or not list(dropins.iterdir()), 'greeter drop-in override present')
    require(p(STATE).parent.is_dir(), 'missing D293 state directory')
    require(not p(GREETER_DROPIN).parent.is_symlink(), 'greeter drop-in parent is a symlink')
    ...
    require(b'\nMODE=normal\n' in payload[RUNTIME + '/PROVENANCE'], 'candidate is not a normal build')
    pam = read(VENDOR_PAM)
    ...
    payload[PAM] = pam.replace(before, f'{RUNTIME}/pam_fprintd.so max-tries=1 timeout=8 debug'.encode())
...
sed 's/^/GOODIX_EARLY_LOGIN_BUILD /' PROVENANCE >&2
exec env LD_LIBRARY_PATH={RUNTIME} FP_DRIVERS_ALLOWLIST=goodix_27c6_5125 {RUNTIME}/fprintd
'''.encode()
    payload[DROPIN] = f'[Service]\nExecStart=\nExecStart={WRAPPER}\n'.encode()
    payload[GREETER_DROPIN] = f'[Service]\nExecStart=\nExecStart={RUNTIME}/greeter\n'.encode()
    state = dict(status='INSTALLING', service=service_state(),
                 greeter_dir_created=not p(GREETER_DROPIN).parent.exists(),
    ...
    try:
        run('systemctl', 'stop', 'fprintd.service')
        require(service_state() == 'inactive', 'service did not stop')
        p(GREETER_DROPIN).parent.mkdir(mode=0o755, exist_ok=True)
        if state['greeter_dir_created']:
            os.chmod(p(GREETER_DROPIN).parent, 0o755)
        p(RUNTIME).mkdir(mode=0o755)
        os.chmod(p(RUNTIME), 0o755)
        for name, data in payload.items():
            write(name, data, 0o755 if name in (WRAPPER, RUNTIME + '/fprintd', RUNTIME + '/greeter') else 0o644)
        for name, target in state['links'].items():
            p(name).symlink_to(target)
        run('restorecon', '-RF', RUNTIME, WRAPPER, DROPIN, GREETER_DROPIN, PAM, STATE)
        # New executable files use the original Fedora entrypoint labels.
        # Their removal also removes these labels; no SELinux policy is changed.
        run('chcon', '--reference=' + DAEMON, RUNTIME + '/fprintd')
        run('chcon', '--reference=/usr/libexec/plasma-login-greeter', RUNTIME + '/greeter')
        run('chcon', '--reference=/usr/lib64/security/pam_fprintd.so', RUNTIME + '/pam_fprintd.so')
        restore_service(state['service'])
        effective(WRAPPER)
        validate_owned(state)
        state['status'] = 'ACTIVE'
        ...
        rollback()
        raise
    print('EARLY_LOGIN_INSTALL=PASS max_tries=1 timeout=8 next=COLD_BOOT rollback=AVAILABLE')
...
if __name__ == '__main__':
    try:
        ...
        print(f'EARLY_LOGIN_TRANSACTION=FAIL {error}', file=sys.stderr)
        sys.exit(1)
