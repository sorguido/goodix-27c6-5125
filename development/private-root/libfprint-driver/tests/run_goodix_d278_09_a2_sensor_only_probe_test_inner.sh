...
build_run_tests () {
  ...
  gcc $strict $extra $cflags $includes \
    ...
    "$root/tools/goodix_d278_a2_sensor_only_probe.c" \
    "$test_dir/test_goodix_d278_09_a2_sensor_only_probe.c" \
...
build_run_tests d278_09_tests ""
build_run_tests d278_09_tests_repeat ""
echo D278_09_NORMAL_DETERMINISM_RUNS=2_PASS
build_run_tests d278_09_tests_sanitized \
  "-O1 -fno-omit-frame-pointer -fsanitize=address,undefined"
echo D278_09_ASAN_UBSAN=PASS

test -f "$host_gusb"
gcc $strict -DD278_09_LIVE_BINDING \
  -DD278_09_APPROVED_BASELINE='"UNAPPROVED_FOR_LIVE"' \
  $cflags $includes \
  "$root/libfprint-driver/goodix_a0_protocol.c" \
  "$root/tools/goodix_d278_a2_sensor_only_probe.c" \
  "$root/tools/d278_a2_sensor_only_observe_once.c" \
  -L"$build" -Wl,-rpath-link,"$build" -l:libgusb.so.2 $libs \
  -Wl,--gc-sections -o "$build/d278_a2_sensor_only_observe_once"

self_test_output=$(LD_LIBRARY_PATH="$build${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
  timeout 30 "$build/d278_a2_sensor_only_observe_once" --self-test)
echo "$self_test_output"
echo "$self_test_output" | grep -F '"goodix_command_count":1' >/dev/null
echo "$self_test_output" | grep -F '"out_submit_count":1' >/dev/null
echo "$self_test_output" | grep -F '"a2_sensor_only_submit_count":1' >/dev/null
echo "$self_test_output" | grep -F '"physical_in_submit_count":2' >/dev/null
echo "$self_test_output" | grep -F '"physical_in_completion_count":2' >/dev/null
echo "$self_test_output" | grep -F '"ack_count":1' >/dev/null
echo "$self_test_output" | grep -F '"typed_response_count":1' >/dev/null
...
LD_LIBRARY_PATH="$build${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
  timeout 30 "$build/d278_a2_sensor_only_observe_once" \
  --live-a2-sensor-only-observe-once >"$build/live_gate.stdout" \
...
runtime_sources="$root/tools/goodix_d278_a2_sensor_only_probe.c $root/tools/d278_a2_sensor_only_observe_once.c"
test "$(grep -h -c 'goodix_a0_build_frame' $runtime_sources | awk '{s += $1} END {print s}')" -eq 3
grep -F 'return goodix_a0_build_frame (0xa2, 0xa2, exact_sensor_only_body,' \
  "$root/tools/goodix_d278_a2_sensor_only_probe.c" >/dev/null
if grep -En '0xe0|0xa4|0xf4|0xd1|0xe4|0xa8|0x70|0x80|0x90' \
   "$root/tools/goodix_d278_a2_sensor_only_probe.c"; then
...
if nm -u "$build/d278_a2_sensor_only_observe_once" | \
   grep -E '(SSL_|goodix_secure_session|g_usb_device_reset|clear_halt|control_transfer)'; then
  echo FORBIDDEN_SYMBOL_AUDIT=FAIL >&2
  exit 1
fi

echo D278_09_FOCAL_TEST_COUNT=9
echo D278_09_LIVE_CAPABLE_LAUNCHER_BUILD=PASS
echo D278_09_LIVE_CAPABLE_LAUNCHER_EXECUTED=false
echo D278_09_UNAPPROVED_LIVE_GATE=PASS_BEFORE_USB_CONTEXT
echo GOODIX_COMMAND_SUBMIT_MAX=1
echo GOODIX_BULK_OUT_SUBMIT_MAX=1
echo A2_SENSOR_ONLY_SUBMIT_MAX=1
echo PHYSICAL_BULK_IN_SUBMIT_MAX=2
echo PHYSICAL_BULK_IN_COMPLETION_MAX=2
echo A8_SUBMIT_MAX=0
