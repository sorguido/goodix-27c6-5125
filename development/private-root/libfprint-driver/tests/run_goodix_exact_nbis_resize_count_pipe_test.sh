...
work=$(mktemp -d /tmp/goodix-nbis-resize-count-pipe.XXXXXX)
cleanup ()
{
  if [[ $work == /tmp/goodix-nbis-resize-count-pipe.* ]]; then
...
flatpak run --user --unshare=network \
  ...
  "$root/libfprint-driver/tests/run_goodix_exact_nbis_resize_count_pipe_test_inner.sh" \
