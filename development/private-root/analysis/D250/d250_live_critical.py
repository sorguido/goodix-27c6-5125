...
"""D250 live-critical Git closure and index-independent baseline verifier."""
...
LIVE_CRITICAL_RATIONALES = {
    "operator_kit/d250-live-af-once.sh": "operator authorization, single-shot lifecycle, patching, cleanup, and reseal",
    ...
    "analysis/D250/D250_af_continuation.patch": "adds the only reachable AF wire action and terminal stop",
    "analysis/D250/d250_preflight.py": "D250 pre-USB safety and baseline policy",
    "analysis/D250/d250_preflight_observability.py": "renders actionable redacted preflight failures",
    "analysis/D250/d250_live_critical.py": "defines and verifies this Git baseline closure",
    "core/post_d4.py": "AF serializer, response validator, semantic version gate, and terminal model",
...
def offline_stale_detection_test() -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="d250-live-critical-") as directory:
        work = Path(directory)
        ...
        commands = (
            ...
            (
                "-c",
                "user.name=D250 Offline Test",
                "-c",
                "user.email=d250-offline@example.invalid",
                "commit",
                "-q",
                "-m",
                "D250 offline baseline fixture",
            ),
        )
    ...
    return {
        "schema": "d250-live-critical-stale-test-v1",
        "status": "PASS",
