#!/usr/bin/env python3
"""Render a redacted, actionable D242 preflight failure summary."""
...
EXPECTED_SCHEMA = "d242-preflight-report-v1"
...
def render(report_path: Path) -> tuple[str, ...]:
    ...
    lines = [
        ...
        "D242_RESULT=FAIL",
        f"D242_FAILURE_CLASS={failure_class}",
        f"D242_PREFLIGHT_FAILURES={failures}",
        f"D242_MARKER_PATH={marker_path}",
        f"D242_USB_OPEN_COUNT={usb_open_count}",
        f"D242_GOODIX_COMMAND_COUNT={goodix_command_count}",
        "D242_REAL_SECRET_READ_COUNT=0",
        "D242_FPRINTD_MUTATION_COUNT=0",
        "D242_LIVE_MARKER_CREATE_COUNT=0",
        "D242_LIVE_USB_EXECUTION=NOT_STARTED",
    ]
    if exception_class is not None:
        lines.append(f"D242_PREFLIGHT_EXCEPTION={exception_class}")
    return tuple(lines)
