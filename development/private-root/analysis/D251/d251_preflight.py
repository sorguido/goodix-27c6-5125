...
"""Minimal D251 root preflight; it never initializes libusb or sends commands."""
...
from analysis.D251.d251_live_critical import baseline_state
...
OUTPUT = REPOSITORY / "analysis/D251/D251_preflight_report.json"
...
D251_MARKER_NAME = "d251-operator-invocation.marker"
...
def d251_runtime_paths() -> ProductionRuntimePaths:
    ...
    return ProductionRuntimePaths(
        ...
        checkpoint_report=reports / "d251-live-pre-restore.json",
        final_report=reports / "d251-live-result.json",
        single_use_marker=store / D251_MARKER_NAME,
    )
...
def marker_namespace_status(store: Path) -> dict[str, object]:
    current = store / D251_MARKER_NAME
    return {
        "namespace": str(current),
        "d251_marker_absent": not os.path.lexists(current),
...
def publish(report: dict[str, object]) -> None:
    ...
    fd, name = tempfile.mkstemp(prefix=".d251-preflight-", dir=OUTPUT.parent)
    temporary = Path(name)
    try:
        os.fchmod(fd, 0o600)
        os.write(fd, payload)
...
def _initial_report() -> dict[str, object]:
    return {
        "schema": "d251-preflight-report-v1",
        "status": "fail",
        "phase": "PREFLIGHT",
        "no_libusb_init": True,
        "libusb_init_count": 0,
...
def offline_sandbox_preflight(root: Path) -> dict[str, object]:
    ...
    passed = bool(
        absent_case["d251_marker_absent"]
        ...
        and not present_case["d251_marker_absent"]
        and directory["pass"]
        and historical_untouched
    )
    return {
        **_initial_report(),
        "schema": "d251-offline-preflight-fixture-v1",
...
def live_preflight() -> int:
    ...
    try:
        ...
        if not markers["d251_marker_absent"]:
            failures.append("d251_single_use_marker_consumed")
        ...
        approved_sha = os.environ.get("D251_APPROVED_LIVE_BASELINE_SHA", "")
...
def main(argv: tuple[str, ...] | None = None) -> int:
    ...
    if arguments:
        print("d251_preflight: unexpected arguments", file=sys.stderr)
        return 64
