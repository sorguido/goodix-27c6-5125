...
"""Executable closure for the source-sealed D251 AF candidate."""
...
from analysis.D251.d251_live_critical import offline_stale_detection_test
...
PATCHES = (
    ...
    Path("analysis/D251/D251_af_semantics_continuation.patch"),
)
...
def run() -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="d251-preflight-fixture-") as directory:
        preflight = offline_sandbox_preflight(Path(directory))
    if preflight.get("status") != "PASS":
        raise RuntimeError("D251 preflight namespace fixture failed")
    rendered_failure = render(
        {
            "failure_class": "d251_single_use_marker_consumed",
            "failures": ["d251_single_use_marker_consumed"],
            "marker_namespace_status": preflight["marker_present_case"],
            "live_critical_set_status": "APPROVED",
        }
    )
    required_lines = (
        "D251_FAILURE_CLASS=D251_SINGLE_USE_MARKER_CONSUMED",
    ...
    if not all(line in rendered_failure for line in required_lines):
        raise RuntimeError("D251 preflight failure observability incomplete")

    with tempfile.TemporaryDirectory(prefix="d251-executable-closure-") as directory:
        work = Path(directory)
        ...
        for step, names in {
            ...
            "D250": ("D250_af_continuation.patch",),
            "D251": ("D251_af_semantics_continuation.patch", "d251_runtime_matrix.py"),
        }.items():
        ...
        for path in (*SOURCE_FILES, Path("core/post_d4.py"), Path("analysis/D251/d251_runtime_matrix.py")):
        ...
        matrix_result = run_checked(
            [sys.executable, "analysis/D251/d251_runtime_matrix.py"],
            cwd=work,
            env=environment,
        )
        matrix = json.loads(matrix_result.stdout)
        if matrix.get("status") != "PASS":
            raise RuntimeError("D251 runtime matrix did not pass")
    ...
    return {
        "schema": "d251-executable-closure-v1",
        "status": "PASS",
        "matrix": matrix,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if arguments != ["--offline-dry-run"]:
        print("D251_SEPARATE_USER_AUTHORIZATION_REQUIRED", file=sys.stderr)
        return 64
