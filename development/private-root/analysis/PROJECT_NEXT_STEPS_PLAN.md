...
`Meta+L → fingerprint → MATCH → unlock` sul runtime storico D293; `sudo`,
login Plasma con password e login Plasma con fingerprint sono anch'essi PASS
umani osservati. D297 è chiuso e la decisione corrente dell'Utente avvia Phase
F. I boundary D279–D297 chiusi non vanno
...
#### Stato Phase F — D298/01 decisione history e separazione del working tree
...
L'Utente ha deciso che la history privata non sarà trasferita né sanitizzata:
resta privata e intatta. L'agente non crea una nuova root history, non tocca il
repository pubblico e prepara soltanto la separazione del working tree. Dopo
l'approvazione editoriale finale l'Utente copierà manualmente i soli file
pubblicabili nel proprio clone pubblico ed eseguirà personalmente commit e
push.

Il lavoro corrente è quindi: rendere build e installazione indipendenti dalla
history privata, spostare il materiale escluso sotto `red_tag/`, riscrivere in
inglese la documentazione product-oriented e validare la superficie rimasta.

```text
D298_01_OUTCOME=USER_DECISION_RECEIVED
PRIVATE_HISTORY_PUBLISHABLE=false
PRIVATE_HISTORY_IS_PUBLICATION_INPUT=false
WORKTREE_RED_TAG_MOVE_SUFFICIENT=true
...
release qualification PASS. La successiva decisione
...
PAM transitoria e il vero `Meta+L` ha prodotto MATCH e unlock. Ha poi riportato
PASS anche per `sudo` con fingerprint e per i login Plasma con password e con
fingerprint. D297 è chiuso; la decisione corrente avvia Phase F senza implicare
...
CURRENT_PHASE=PHASE_F
PRODUCTION_READY=false
RELEASE_CANDIDATE_READY=READY_FOR_PHASE_F_PUBLICATION_PREPARATION
